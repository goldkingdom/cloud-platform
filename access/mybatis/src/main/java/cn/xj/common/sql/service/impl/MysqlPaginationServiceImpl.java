 ...
 */
@Service
public class MysqlPaginationServiceImpl implements PaginationService {
    ...
    public List query(StringBuffer sql, Map params, Map pagination, String countColumn) {
        ...
        //根据分页信息进行分页查询
        StringBuffer paginationSql = sql.append(" limit {offset,Integer},{pageSize,Integer}");
        params = params == null ? Maps.newHashMap() : params;
        params.put("offset", (ConvertUtil.objectToInt(pagination.get("currentPage")) - 1) * ConvertUtil.objectToInt(pagination.get("itemsPerPage")));
        params.put("pageSize", pagination.get("itemsPerPage"));
        List<Map> list = sqlService.query(paginationSql, params);
        str = null;
        return list;
    }
